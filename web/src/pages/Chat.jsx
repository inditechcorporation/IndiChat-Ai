...
import {
  ...
  Bot, Sparkles, Globe, ChevronRight, Lock, MessageSquare
} from 'lucide-react';
...
const MODELS = [
  ...
  { id: 'moonshotai/kimi-k2-instruct',               label: 'Kimi K2',          provider: 'groq',     color: '#f97316', free: true,  vision: false },
...
export default function Chat({ user }) {
  ...
  const send = async (text) => {
    ...
    const userMsg = { role: 'user', content: msgText, id: Date.now(), image: image?.preview };
    ...
    try {
      ...
      setMessages([...history, { role: 'assistant', content: reply, id: Date.now() + 1, ms: elapsed }]);
      if (autoSpeak) speak(reply);
    } catch (e) {
      setResponseTime(Date.now() - t0);
      setMessages([...history, { role: 'assistant', content: `⚠️ ${e.message}`, id: Date.now() + 1, error: true }]);
  ...
  return (
    <div style={{ ...pg, display: 'flex', height: '100vh' }}>
        ...
        <div style={{ width: '240px', background: t.bg2, borderRight: `1px solid ${t.border}`, display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
          <div style={{ padding: '16px', borderBottom: `1px solid ${t.border}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <div>
                <div style={{ fontWeight: '700', fontSize: '14px', background: `linear-gradient(135deg, ${t.accent}, ${t.accent2})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>IndiChat-Ai</div>
          ...
          <div style={{ marginTop: 'auto', padding: '12px', borderTop: `1px solid ${t.border}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '8px', background: `${model.color}18`, borderRadius: '8px', border: `1px solid ${model.color}33` }}>
              <div style={{ width: 28, height: 28, background: `${model.color}22`, borderRadius: '7px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <ProviderIcon provider={model.provider} size={14} />
              </div>
              <div>
                <div style={{ fontSize: '12px', fontWeight: '600' }}>{model.label}</div>
                <div style={{ fontSize: '10px', color: model.free ? '#22c55e' : t.text2 }}>
                  {model.free ? 'Free · Groq' : model.provider}
                </div>
              </div>
            </div>
          </div>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        {/* Top bar */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', borderBottom: `1px solid ${t.border}`, background: t.bg }}>
          <button onClick={() => setSidebarOpen(!sidebarOpen)} style={iconBtn}><Menu size={18} /></button>
          <div style={{ flex: 1, fontWeight: '600', fontSize: '14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <div style={{ width: 24, height: 24, background: `${model.color}22`, borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <ProviderIcon provider={model.provider} size={13} />
            </div>
            {model.label}
            {model.free && <span style={{ fontSize: '10px', color: '#22c55e', background: '#22c55e22', padding: '2px 6px', borderRadius: '4px', border: '1px solid #22c55e33' }}>FREE</span>}
            {model.vision && <span style={{ fontSize: '10px', color: '#4285f4', background: '#4285f422', padding: '2px 6px', borderRadius: '4px', border: '1px solid #4285f433', display: 'flex', alignItems: 'center', gap: '3px' }}><Eye size={10} /> Vision</span>}
          </div>
          {/* Language selector */}
          <select
            value={voiceLang}
            ...
            style={{ background: t.bg2, border: `1px solid ${t.border}`, color: t.text, borderRadius: '6px', padding: '4px 8px', fontSize: '12px', cursor: 'pointer', outline: 'none' }}>
          ...
          <button onClick={() => setMessages([])} style={iconBtn} title="Clear"><Trash2 size={16} /></button>
        </div>

        {/* Messages */}
        <div style={{ flex: 1, overflowY: 'auto', padding: '24px 0' }}>
          {messages.length === 0 && (
            <div style={{ textAlign: 'center', marginTop: '80px', color: t.text2 }}>
              <div style={{ width: 64, height: 64, background: `${model.color}22`, border: `1px solid ${model.color}44`, borderRadius: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', animation: 'float 4s ease-in-out infinite' }}>
                <ProviderIcon provider={model.provider} size={32} />
              </div>
              <div style={{ fontSize: '20px', fontWeight: '600', color: t.text, marginBottom: '8px' }}>How can I help you?</div>
              <div style={{ fontSize: '13px', color: t.text2 }}>IndiChat-Ai · {model.label}</div>
            </div>
          )}
          {messages.map(msg => (
            <div key={msg.id} style={{ maxWidth: '720px', margin: '0 auto', padding: '8px 24px' }}>
              {msg.role === 'user' ? (
                ...
                <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-start' }}>
                  <div style={{ width: '28px', height: '28px', background: `${model.color}22`, border: `1px solid ${model.color}44`, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: '2px' }}>
                    <ProviderIcon provider={model.provider} size={13} />
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: '14px', lineHeight: 1.7, color: msg.error ? t.danger : t.text, whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>
                      {msg.content}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginTop: '6px' }}>
                      {!msg.error && (
                        <button onClick={() => speak(msg.content)} style={{ background: 'transparent', border: 'none', color: t.text2, cursor: 'pointer', fontSize: '12px', padding: 0, display: 'flex', alignItems: 'center', gap: '4px' }}>
                          <Volume2 size={13} /> Speak
                        </button>
                      )}
                      {msg.ms && (
                        <span style={{ fontSize: '11px', color: t.text3 }}>
                          {msg.ms < 1000 ? `${msg.ms}ms` : `${(msg.ms/1000).toFixed(1)}s`}
                        </span>
                      )}
                    </div>
                  </div>
          ...
          {loading && (
            <div style={{ maxWidth: '720px', margin: '0 auto', padding: '8px 24px', display: 'flex', gap: '12px', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                {[0,1,2].map(i => <div key={i} style={{ width: '6px', height: '6px', background: t.text2, borderRadius: '50%', animation: `pulse 1.4s ${i*0.2}s infinite` }} />)}
                <span style={{ fontSize: '11px', color: t.text3, marginLeft: '8px' }}>Thinking...</span>
              </div>
            </div>
          )}
          <div ref={bottomRef} />
        ...
        <div style={{ padding: '16px 24px', borderTop: `1px solid ${t.border}` }}>
          ...
          {image && (
            <div style={{ maxWidth: '720px', margin: '0 auto 8px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              <img src={image.preview} alt="preview" style={{ height: '60px', borderRadius: '8px', border: `1px solid ${t.border}` }} />
...
const btn      = { padding: '12px 20px', background: `linear-gradient(135deg, ${t.accent}, ${t.accent2})`, color: '#fff', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' };
...
const iconBtn  = { background: 'transparent', border: 'none', color: t.text2, cursor: 'pointer', fontSize: '16px', padding: '6px', borderRadius: '6px' };
